...
func (f *Funcs) FuncMap() template.FuncMap {
	return template.FuncMap{
		...
		"querystr": f.querystr,
		"sqlstr":   f.sqlstr,
		"sqlstr_select":   f.sqlstr_select,
		"sqlstr_update_deleted_at": f.sqlstr_update_deleted_at,
		"sqlstr_update_deleted_at_without_updated_at": f.sqlstr_update_deleted_at_without_updated_at,
		// helpers
...
func (f *Funcs) funcfn(name string, context bool, v interface{}) string {
	...
	case gotpl.Index:
		...
		storageName = x.Table.GoName
		r = append(r, rt)
	default:
		return fmt.Sprintf("[[ UNSUPPORTED TYPE 3: %T ]]", v)
	}
	r = append(r, "error")
	return fmt.Sprintf("func (s *%sStorage) %s(%s) (%s)", storageName, name, strings.Join(p, ", "), strings.Join(r, ", "))
}

// func_context generates a func signature for v with context determined by the
// context mode.
func (f *Funcs) func_context(v interface{}) string {
	return f.funcfn("Get" + f.func_name_context(v), f.contextfn(), v)
}
...
func (f *Funcs) db_prefix(name string, skip bool, vs ...interface{}) string {
	...
	for i, v := range vs {
		...
		case gotpl.Table:
			...
			ignore = append(ignore, "CreatedAt", "UpdatedAt", "DeletedAt")
...
func (f *Funcs) logf_pkeys(v interface{}) string {
	...
	return fmt.Sprintf("logger.Debugf(ctx, %s)", strings.Join(p, ", "))
}
...
func (f *Funcs) logf(v interface{}, ignore ...interface{}) string {
	...
	switch x := v.(type) {
	case gotpl.Table:
		p = append(p, f.names_ignore("item.", x, ignoreNames...))
	default:
...
func (f *Funcs) sqlstr_insert_base(all bool, v interface{}) []string {
	...
	case gotpl.Table:
		...
		for _, z := range x.Fields {
			...
			switch f.colname(z) {
				case "updated_at", "deleted_at":
				case "created_at":
					fields, vals = append(fields, f.colname(z)), append(vals, "UTC_TIMESTAMP()")
				default:
	...
	case gotpl.Table:
		...
		for _, z := range x.Fields {
			...
			switch f.colname(z) {
				...
				case "updated_at":
					list = append(list, fmt.Sprintf("%s = %s", name, "UTC_TIMESTAMP()"))
				default:
...
// sqlstr_select builds a select query.
func (f *Funcs) sqlstr_select(v interface{}) string {
	switch x := v.(type) {
	case gotpl.Table:
		// build table fieldnames
		var fields []string
		...
		return "SELECT " + strings.Join(fields, ", ") + " FROM " + x.SQLName
	}
	return fmt.Sprintf("[[ UNSUPPORTED TYPE 27: %T ]]", v)
}

// sqlstr_update_deleted_at builds a update query.
func (f *Funcs) sqlstr_update_deleted_at(v interface{}) string {
	switch x := v.(type) {
	case gotpl.Table:
		return "UPDATE " + x.SQLName + " SET updated_at = UTC_TIMESTAMP(), deleted_at = UTC_TIMESTAMP() WHERE id = ?"
	}
	return fmt.Sprintf("[[ UNSUPPORTED TYPE 27: %T ]]", v)
}

// sqlstr_update_deleted_at_without_updated_at builds a update query.
func (f *Funcs) sqlstr_update_deleted_at_without_updated_at(v interface{}) string {
	switch x := v.(type) {
	case gotpl.Table:
		return "UPDATE " + x.SQLName + " SET deleted_at = UTC_TIMESTAMP() WHERE id = ?"
	}
	return fmt.Sprintf("[[ UNSUPPORTED TYPE 27: %T ]]", v)
}
...
// hasfield return true if field exists
func (f *Funcs) hasfield(field string, v interface{}) bool {
	switch x := v.(type) {
	case gotpl.Table:
		for _, z := range x.Fields {
			if z.GoName == field {
